                    ...
                    <div className="h-[50vh] md:h-[65vh] rounded-[32px] md:rounded-[40px] bg-background border border-white/5 flex items-end p-6 sm:p-12 md:p-20 relative overflow-hidden group shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)]">
                        ...
                        <div className="relative z-10 max-w-4xl animate-in fade-in slide-in-from-bottom-8 duration-700">
                            <div className="flex items-center gap-3 mb-6">
                                <span className="bg-primary text-background px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] shadow-lg shadow-primary/20">
                                    Suggested Reading
                                </span>
                                <div className="h-[1px] w-12 bg-white/10"></div>
                            </div>

                            <h1 className="text-4xl sm:text-6xl md:text-8xl font-black text-white mb-6 tracking-tighter leading-[0.9] uppercase drop-shadow-2xl">{items[0].title}</h1>
                            ...
                            <div className="flex flex-wrap items-center gap-6 text-[10px] font-black uppercase tracking-widest mb-10 opacity-70">
                                <div className="flex items-center gap-2 text-primary">
                                    <span className="text-xs">★</span> {items[0].score || '8.2'}
                                </div>
                            ...
                            <p className="text-textMuted mb-12 line-clamp-2 leading-relaxed text-sm md:text-lg md:w-2/3 opacity-80 tracking-tight">
                                {items[0].synopsis || 'The story of an extraordinary journey into a world of complex characters and high stakes.'}
                            </p>

                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                                <Link to={`/manga/read/${items[0].mal_id}`} className="bg-primary text-background font-black py-4.5 px-12 rounded-2xl shadow-xl shadow-primary/10 hover:bg-primaryDark transition-all flex items-center justify-center gap-3 text-sm active:scale-95 uppercase tracking-widest">
                                    Start Reading
                                </Link>
                                <Link to={`/manga/${items[0].mal_id}`} className="bg-white/5 backdrop-blur-xl text-white font-black py-4.5 px-12 rounded-2xl border border-white/10 hover:bg-white/10 transition-all text-sm flex items-center justify-center uppercase tracking-widest">
                                    Details
                                </Link>
                            </div>
                        </div>
                ...
                <aside className="w-full md:w-72 flex-shrink-0">
                    <div className="bg-card border border-white/5 rounded-[32px] p-8 md:sticky md:top-28 shadow-2xl overflow-hidden group">
                        ...
                        <div className="flex md:flex-col overflow-x-auto md:overflow-x-visible pb-2 md:pb-0 gap-3 scrollbar-hide">
                            <label className="hidden md:block text-[11px] font-bold text-textMuted uppercase tracking-widest mb-4 ml-1 opacity-40">Categories</label>
                            {[
                ...
                <main className="flex-1">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-14">
                        ...
                        <div className="relative group w-full lg:w-96">
                            ...
                            <input
                                ...
                                className="w-full bg-card border border-white/5 text-white rounded-2xl py-5 pl-14 pr-12 outline-none focus:border-primary/50 focus:bg-white/[0.05] transition-all text-xs font-bold uppercase tracking-widest placeholder:text-textMuted/40 shadow-inner"
                            />
                            {searchQuery && (
                                <button
                    ...
                    {items.length === 0 && !loading ? (
                        <div className="py-32 flex flex-col items-center text-center bg-card rounded-[48px] border border-white/5 shadow-inner">
                            ...
                            <h3 className="text-3xl font-bold text-white mb-3 tracking-tighter uppercase">No Results</h3>
                            <p className="text-textMuted text-[10px] font-bold uppercase tracking-wider max-w-xs opacity-60">Try searching for something else</p>
                        </div>
                    ) : (
