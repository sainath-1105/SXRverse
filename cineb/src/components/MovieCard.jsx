...
export default function MovieCard({ item, type }) {
    ...
    const ratingColor = ratingNum >= 8.0 ? 'text-primary border-primary/30 bg-primary/10 ' :
        ratingNum >= 6.0 ? 'text-primary/70 border-primary/20 bg-primary/5' :
            ratingNum > 0 ? 'text-accent border-accent/20 bg-accent/5' :
                'text-textMuted/40 border-white/5 bg-white/5';

    const poster = getImageUrl(item.poster_path, 'w342');

    return (
        <Link to={`/watch/${t}/${item.id}`} className="group relative block w-full overflow-hidden rounded-[24px] bg-card border border-white/5 transition-all duration-700 hover:scale-[1.03] hover:-translate-y-2 cursor-pointer shadow-2xl hover:shadow-primary/5">
            <div className="relative aspect-[2/3] w-full bg-background overflow-hidden">
                ...
                <div className="absolute top-4 left-4 z-20">
                    <div className="bg-background/60 backdrop-blur-xl border border-white/10 text-white/50 px-3 py-1.5 rounded-xl font-black text-[7px] uppercase tracking-[0.3em] shadow-2xl">
                        {t}
                    </div>
                </div>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-700 flex items-center justify-center backdrop-blur-[1px]">
                    <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center text-background shadow-[0_0_50px_rgba(251,191,36,0.3)] transform scale-50 group-hover:scale-100 transition-all duration-500 ease-out">
                        <Play size={24} className="fill-current translate-x-0.5" />
                    </div>
                </div>
            </div>

            <div className="p-5 bg-gradient-to-b from-card to-background">
